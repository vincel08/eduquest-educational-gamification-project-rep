...
async function main() {
  ...
  try {
    ...
    const [usersAfter] = await connection.query('SELECT COUNT(*) AS total FROM users');
    const [courses] = await connection.query('SELECT COUNT(*) AS total FROM courses');
    const [quizzes] = await connection.query('SELECT COUNT(*) AS total FROM quizzes');
    const [games] = await connection.query('SELECT COUNT(*) AS total FROM educational_games');
    ...
    console.log(`  student_profiles    : ${profiles[0].total} (kept, XP sum=${profiles[0].xp_sum})`);
