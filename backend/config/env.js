...
function assertValidClientUrl(rawUrl) {
  ...
  if (value === '*' || value.includes('*')) {
    throw new Error('CLIENT_URL must be an exact origin in production (wildcards are not allowed)');
  }
  ...
  try {
    ...
    throw new Error('CLIENT_URL must be a valid absolute URL (e.g. https://app.example.com)');
  }
...
if (isProduction) {
  ...
  if (!process.env.PORT) {
    throw new Error('Missing required environment variable in production: PORT');
  }
