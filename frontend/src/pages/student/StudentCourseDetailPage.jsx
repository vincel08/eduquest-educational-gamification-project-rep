...
export default function StudentCourseDetailPage() {
  ...
  const load = useCallback(async () => {
    try {
      const [courseRes, lessonsRes, quizzesRes, gamesRes, myCoursesRes] = await Promise.all([
      ...
      if (isEnrolled) {
        try {
          const eligibilityRes = await gamificationService.getCourseCertificateEligibility(courseId);
  ...
  async function handleEnroll() {
    ...
    try {
      ...
      setMessage('Enrolled successfully. You can now start lessons and quizzes.');
      setLoading(true);
      await load();
    } catch (err) {
      setError(getErrorMessage(err));
      ...
      <PageHeader
        ...
        action={(
          canEnroll ? (
            ...
            <Chip color="success" icon={<CheckCircleIcon />} label="Enrolled" sx={{ fontWeight: 800, bgcolor: 'rgba(255,255,255,0.92)' }} />
          ) : null
        )}
      />
      <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap sx={{ mb: 2 }}>
        {course.grade_level ? <Chip size="small" label={course.grade_level} color="secondary" variant="outlined" /> : null}
        {course.subject ? <Chip size="small" label={course.subject} variant="outlined" /> : null}
        <Chip size="small" icon={<MenuBookIcon />} label={`${lessons.length} lessons`} />
        <Chip size="small" icon={<QuizIcon />} label={`${quizzes.length} quizzes`} />
        <Chip size="small" icon={<SportsEsportsIcon />} label={`${games.length} games`} />
      </Stack>
      ...
      <Stack spacing={2}>
        {enrolled ? (
          <Paper sx={{ p: { xs: 2, md: 3 } }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5 }}>
              Based on completed lessons only. Reaching 100% does not mean your certificate is ready.
            </Typography>
        ...
        {enrolled ? (
          <Paper sx={{ p: { xs: 2, md: 3 } }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Certificate eligibility is separate from learning progress. Games are optional.
            </Typography>
            ...
            >
              <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 0.75 }}>
                ...
                <Typography fontWeight={800}>
                  Certificate
                  {' · '}
                  {certificateStatus.locked ? '🔒 Complete required items' : '✓ Available'}
                </Typography>
              </Stack>
          ...
          <Alert severity="info">
            Enroll in this course to track learning progress and unlock certificate requirements.
          </Alert>
        ) : null}
            ...
            {lessons.map((lesson) => (
              ...
              >
                <ListItemText
                  ...
                  secondary={(
                    <Stack spacing={0.5} sx={{ mt: 0.5, pr: { xs: 0, sm: 12 } }}>
                      <Typography variant="body2" color="text.secondary">
        ...
        <Paper sx={{ p: { xs: 2, md: 3 } }}>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
            Required quizzes count toward certificate eligibility, not the lesson progress bar.
          </Typography>
          <List>
            {quizzes.map((quiz) => {
          ...
          {!quizzes.length ? (
            <Typography color="text.secondary">No quizzes published for this course.</Typography>
          ) : null}
        </Paper>
