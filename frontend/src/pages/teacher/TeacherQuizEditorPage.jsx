...
export default function TeacherQuizEditorPage() {
  ...
  const [questions, setQuestions] = useState([blankQuestion('multiple_choice')]);
  ...
  useEffect(() => {
    ...
    quizService.getById(quizId)
      .then((response) => {
        ...
        setQuestions(mapped.length ? mapped : [blankQuestion('multiple_choice')]);
      })
      .catch((err) => {
  ...
  async function uploadPendingImages(quiz) {
    ...
    for (let i = 0; i < questions.length; i += 1) {
      ...
      if (local?.questionType === 'image_question' && local.imageFile && saved?.id) {
  ...
  return (
    ...
    <Stack spacing={3}>
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} justifyContent="flex-end">
        <Button variant="outlined" disabled={saving} onClick={saveDraft} startIcon={<SaveIcon />}>
          Save Draft
        </Button>
        <Button variant="outlined" disabled={saving} onClick={handlePreview} startIcon={<VisibilityIcon />}>
          Preview
        </Button>
        <Button variant="contained" disabled={saving} onClick={handlePublish} startIcon={<PublishIcon />}>
          Publish
        </Button>
      </Stack>
