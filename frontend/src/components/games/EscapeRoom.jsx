...
function DoorVisual({ doorOpen, isLocked, size = 'md' }) {
  const isCompact = size === 'sm';
  return (
    <Box
      sx={{
        position: 'relative',
        width: isCompact ? 120 : { xs: 130, sm: 180 },
        height: isCompact ? 180 : { xs: 200, sm: 280 },
        mx: 'auto',
        perspective: 900,
        flexShrink: 0,
      }}
    >
      ...
      <MotionBox
        ...
        sx={{
          ...
          inset: 0,
          background:
            'radial-gradient(circle at 50% 40%, #F8E7B0 0%, #E8C56A 45%, #8B6B2E 100%)',
          boxShadow: '0 0 40px rgba(248, 231, 176, 0.55)',
        }}
      />

      <MotionBox
        animate={{ rotateY: doorOpen ? -78 : 0 }}
...
function CodePanel({
  ...
  return (
    ...
    >
      <Typography
        variant="caption"
        sx={{ color: 'rgba(245,230,200,0.8)', fontWeight: 700 }}
      >
      ...
      <Button
        variant="contained"
        fullWidth
        disabled={feedbackOpen || unlocking || doorOpen || !draft.trim()}
        onClick={submit}
        sx={{
          bgcolor: '#8B5E3C',
          color: '#F5E6C8',
...
export default function EscapeRoom({ gameData, onComplete, xpReward = 50 }) {
  ...
  const inputDisabled = Boolean(feedback?.open || unlocking || doorOpen);
  ...
  function submit() {
    ...
    const nextScore = isCorrect
      ? Math.min(100, score + Math.round(100 / stages.length))
      : score;
    ...
    showFeedback({
      ...
      explanation: isCorrect
        ? null
        : current.hint || `The code is related to: ${current.clue}`,
    ...
    <Stack spacing={2}>
      ...
      >
        ...
        <Stack direction="row" spacing={0.75} sx={{ ml: 'auto' }}>
          {stages.map((_, stageIndex) => {
            const cleared =
              stageIndex < index || (stageIndex === index && doorOpen);
            const currentRoom = stageIndex === index;
            return (
              <Box
                key={`door-marker-${stageIndex}`}
                title={
                  cleared ? 'Unlocked' : currentRoom ? 'Current room' : 'Locked'
                }
                sx={{
                  width: 28,
                  height: 36,
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {/* ===== Mobile: stacked flow (no overlap / clipping) ===== */}
          <Stack
            spacing={1.5}
            sx={{
          ...
          >
            ...
            >
              <Typography
                variant="caption"
                fontWeight={800}
                sx={{ letterSpacing: 0.6, display: 'block', mb: 0.5 }}
              >
            ...
            <DoorVisual doorOpen={doorOpen} isLocked={isLocked} size="sm" />
          ...
          {/* ===== Desktop / tablet: cinematic absolute layout ===== */}
          <Box
            sx={{
          ...
          >
            ...
            >
              <DoorVisual doorOpen={doorOpen} isLocked={isLocked} size="md" />
            </Box>
